...
class ViolationOverlay:
    ...
    def show(self, rule_name: str, app_name: str, restriction_end: Optional[dtime] = None) -> None:
        end_str = restriction_end.strftime("%H:%M") if restriction_end else "—"
        msg = f"Rule: {rule_name}   ·   Until: {end_str}   ·   Blocked: {app_name}"
        self._root.after(0, lambda: self._do_show(msg))
    ...
    def _build_window(self) -> None:
        ...
        tk.Button(inner, text="Emergency Override…",
                  bg="#2d2050", fg="#c8a0ff",
                  activebackground="#3a2a6a", activeforeground="#e0c0ff",
                  relief="flat", bd=0, padx=10, pady=3,
                  font=("Segoe UI", 8, "bold"),
                  cursor="hand2",
                  command=self._override).place(relx=1.0, rely=0.0, anchor="ne",
                                                x=-2, y=2)

        self._win = win

    def _do_show(self, msg: str) -> None:
        self._build_window()
        if self._label_var:
